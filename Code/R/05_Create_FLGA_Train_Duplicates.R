...
create_FLGA_duplicates <- function(duplicate_count) {
  
  ## --------------------
  ## Load Data
  ## --------------------
  
  data <- fread("../../Results/predictions_FLGA_test_FLGA_train_s.csv")
  data$id <- 1:nrow(data)
  
  data$error <- NA
  data$error[data$race == "White"] <- 1 - data$White[data$race == "White"]
  data$error[data$race == "Asian"] <- 1 - data$Asian[data$race == "Asian"]
  data$error[data$race == "Black"] <- 1 - data$Black[data$race == "Black"]
  data$error[data$race == "Hispanic"] <- 1 - data$Hispanic[data$race == "Hispanic"]
  
  q1 <- quantile(data$error[data$race == "Asian"], p = 0.99)
  q2 <- quantile(data$error[data$race == "Hispanic"], p = 0.99)
  q3 <- quantile(data$error[data$race == "White"], p = 0.99)
  q4 <- quantile(data$error[data$race == "Black"], p = 0.99)
  
  data.sub.1 <- data[data$error >= q1 & data$race == "Asian", ]
  data.sub.2 <- data[data$error >= q2 & data$race == "Hispanic", ]
  data.sub.3 <- data[data$error >= q3 & data$race == "White", ]
  data.sub.4 <- data[data$error >= q4 & data$race == "Black", ]
  
  data.sub <- data.table(rbind(data.sub.1[sample(1:nrow(data.sub.1), 75), ], 
                               data.sub.2[sample(1:nrow(data.sub.2), 75)],
                               data.sub.3[sample(1:nrow(data.sub.3), 75)], 
                               data.sub.4[sample(1:nrow(data.sub.4), 75)]))
  
  data.sub0 <- data[!(data$id %in% data.sub$id), ]
  data.sub0$dup <- 0
  data.sub$dup <- 1
  
  data.sub.x <- list()
  for(i in 1:duplicate_count) {
    data.sub.x[[i]] <- data.sub
  }
  data.sub.dup.x <- do.call('rbind', data.sub.x)
  
  data.new.x <- data.table(rbind(data.sub0[, c("first_name", "last_name", "race", "id", "dup")],
                                 data.sub.dup.x[, c("first_name", "last_name", "race", "id", "dup")]))
  
  train.data <- data.new.x
  ...
  train.data <- train.data[, c('id', 'first_name', 'last_name', 'race', 
                               ...
                               'last_name_low', 'first_name_low', 'dup'), 
                           with = F]
  
  train.data <- train.data[dup == 1, ]
  output_file_path <- sprintf("../../Data/Duplicates/FLGA_dups%s.csv", duplicate_count)
